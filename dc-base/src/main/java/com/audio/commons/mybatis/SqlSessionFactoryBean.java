 ...
 * <p>
 * (c) 2014-2016 kernelstudio <service@kernelstudio.com>
 * <p>
 * For the full copyright and license information, please view the LICENSE file
 * that was distributed with this source code.
 *
 * @author kernelstudio < service@kernelstudio.com >
 * @link http://www.kernelstudio.com
 * @version 0.1
 * @since 0.1
 */
package com.audio.commons.mybatis;
        ...
        ApplicationListener<ApplicationEvent>
{
    ...
    private String antTypeAliasesLocations;

    private Map<String, Class<?>> antTypeAliasesMap;
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws IOException
    {
        ...
        // 扩展ant风格的别名配置
        buildAntTypeAliasesPackage(configuration);
        if (null != this.antTypeAliasesMap && !this.antTypeAliasesMap.isEmpty())
        {
            for (Entry<String, Class<?>> typeAlias : this.antTypeAliasesMap.entrySet())
            {
                configuration.getTypeAliasRegistry()
                        .registerAlias(typeAlias.getKey(),
                                typeAlias.getValue());
                if (LOGGER.isDebugEnabled())
                {
                    LOGGER.debug("Registered ant type alias: '" +
                            typeAlias.getKey() + "[" +
                            typeAlias.getValue().getName() + "]" + "'");
                }
            }
        }
        // end
        ...
        if (this.databaseIdProvider != null)
        {
            ...
            try
            {
                configuration.setDatabaseId(this.databaseIdProvider.getDatabaseId(
                        this.dataSource));
            }
        ...
        if (!isEmpty(this.mapperLocations))
        {
            ...
            for (Resource mapperLocation : this.mapperLocations)
            {
                ...
                if (LOGGER.isDebugEnabled())
                {
                    LOGGER.debug("Parsed dao file: '" + mapperLocation + "'");
                }
            }
        }
        else
     ...
     * @throws IOException
     */
    private Map<String, Class<?>> buildAntTypeAliasesPackage(
            Configuration configuration) throws IOException
    {
        if (antTypeAliasesLocations != null)
        {
            Configuration aliasesConfiguration = null;
            XMLConfigBuilder xmlConfigBuilder = null;
            ResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
            antTypeAliasesMap = new HashMap<String, Class<?>>();
            String[] aliasesPackages = antTypeAliasesLocations.split(",");
            for (int i = 0; i < aliasesPackages.length; i++)
            {
                String aliases = aliasesPackages[i];
                aliases = (aliases + "").trim();
                LOGGER.debug(
                        "Parsed type aliases package file: [" + aliases + "]");
                if (aliases != null && aliases.length() > 0)
                {
                    Resource[] aliasesLocations = resolver.getResources(aliases);
                    if (!isEmpty(aliasesLocations))
                    {
                        for (Resource aliasesLocation : aliasesLocations)
                        {
                            if (aliasesLocation == null)
                            {
                                continue;
                            }
                            try
                            {
                                if (aliasesLocation != null)
                                {
                                    xmlConfigBuilder = new XMLConfigBuilder(
                                            aliasesLocation.getInputStream(),
                                            null,
                                            this.configurationProperties);
                                    xmlConfigBuilder.parse();
                                    aliasesConfiguration = xmlConfigBuilder.getConfiguration();
                                }
                                if (null != aliasesConfiguration)
                                {
                                    TypeAliasRegistry aliasesRegistry = aliasesConfiguration
                                            .getTypeAliasRegistry();
                                    for (Entry<String, Class<?>> alias : aliasesRegistry
                                            .getTypeAliases()
                                            .entrySet())
                                    {
                                        antTypeAliasesMap.put(alias.getKey(),
                                                alias.getValue());
                                    }
                            ...
                            {
                                LOGGER.debug(
                                        "Parsed dao file: '" + aliasesLocation +
                                                "'");
                            }
                        }
                    }
    ...
    /**
     * Set locations of MyBatis dao files that are going to be merged into
    ...
    public void setAntTypeAliasesLocations(String antTypeAliasesLocations)
    {
        this.antTypeAliasesLocations = antTypeAliasesLocations;
    }
