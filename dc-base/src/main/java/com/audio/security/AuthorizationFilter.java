...
public class AuthorizationFilter extends AbstractSecurityInterceptor implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		 ...
		 * 
		 * 1.获取请求资源的权限 : MySecurityMetadataSource Collection<ConfigAttribute>
		 * attributes = securityMetadataSource.getAttributes(fi);
		 * 
		 * 2.是否拥有权限,获取安全主体，可以强制转换为UserDetails的实例 : MyAccessDecisionManager
		 * Authentication authenticated = authenticateIfRequired();
		 * this.accessDecisionManager.decide(authenticated, fi, attributes);
		 * 
		 * 2. 用户拥有的权限GrantedAuthority Collection<GrantedAuthority>
